...
export class CompilerWithImport {
  ...
  private checkTreeMatching(
    ...
    if (current.virtual) {
      ...
      for (const v of virtuals) {
        if (
          this.isEq(current.funContent, v, eq) ||
          this.isDep0(v, current.funContent, dep)
        ) {
          return undefined;
        }
      }
      ...
      for (const v of virtuals) {
        if (
          this.isEq(target.funContent, v, eq) ||
          this.isDep0(v, target.funContent, dep)
        ) {
          return undefined;
        }
      }
  ...
  private matchTermOpCNode1(
    ...
    const virtuals = [
      ...this.getVirtualOfTermOpCNode2(current),
      ...this.getVirtualOfTermOpCNode2(target),
    ];
